      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-md bg-white rounded-2xl shadow-xl border border-slate-100">
        <div className="p-8 lg:p-10 space-y-8">
          <div className="flex flex-col items-center gap-4 text-center">
            <div className="p-4 bg-blue-600 rounded-2xl shadow-lg shadow-blue-100"><BarChart3 className="w-8 h-8 text-white" /></div>
            <div>
            ...
            <div className="space-y-4">
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  ...
                  <input ref={usernameRef} type="text" placeholder="Username..." className={`w-full pl-12 pr-4 py-3 bg-slate-50 border rounded-xl text-sm outline-none transition-all ${errors.username ? 'border-rose-500' : 'border-slate-100 focus:border-blue-500'}`} value={formData.username} onChange={(e) => handleInputChange('username', e.target.value)} />
                </div>
              ...
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  ...
                  <input ref={passwordRef} type="password" placeholder="••••••••" className={`w-full pl-12 pr-4 py-3 bg-slate-50 border rounded-xl text-sm outline-none transition-all ${errors.password ? 'border-rose-500' : 'border-slate-100 focus:border-blue-500'}`} value={formData.password} onChange={(e) => handleInputChange('password', e.target.value)} />
                </div>
                {errors.password && <p className="text-[11px] text-rose-500 font-bold px-1 mt-1">{errors.password}</p>}
              </div>
            </div>
            <button disabled={loading} type="submit" className="w-full py-3.5 bg-blue-600 text-white rounded-xl text-sm font-bold shadow-lg hover:bg-blue-700 transition-all flex items-center justify-center gap-2 cursor-pointer">{loading ? <Loader2 className="w-5 h-5 animate-spin" /> : <>Đăng nhập ngay <ArrowRight className="w-4 h-4" /></>}</button>
          </form>
          <div className="pt-6 border-t border-slate-100 text-center">
            <p className="text-slate-400 text-sm font-medium">Chưa có tài khoản? <Link to="/register" className="text-blue-600 font-bold hover:underline">Đăng ký thành viên</Link></p>
          </div>
        </div>
      </motion.div>
    </div>
